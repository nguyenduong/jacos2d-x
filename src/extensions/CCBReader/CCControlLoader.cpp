...
void CCControlLoader::onHandlePropTypeBlockCCControl(CCNode * pNode, CCNode * pParent, const char * pPropertyName, BlockCCControlData * pBlockCCControlData, CCBReader * pCCBReader) {
    if(strcmp(pPropertyName, PROPERTY_CCCONTROL) == 0) {
		if (pBlockCCControlData->mSelectorName.length() == 0) {
        ...
        } else if (pCCBReader->isJSControlled()){
            ((CCControl *)pNode)->addTargetWithActionForControlEvents(pCCBReader->getAnimationManager()->getJSController(), pBlockCCControlData->mSelectorName.getCString(), pBlockCCControlData->mControlEvents);
		} else {
			//v8::Handle<v8::Object> func = this->getJSCCallFunc()->getObjectMemberByName(pBlockCCControlData->mTarget->getJSObjInstance(), pBlockCCControlData->mSelectorName.getCString());
			((CCControl *)pNode)->addTargetWithActionForControlEvents(pBlockCCControlData->mTarget->getJSObjInstance(), pBlockCCControlData->mSelectorName.getCString(), pBlockCCControlData->mControlEvents);
		}
    } else {
