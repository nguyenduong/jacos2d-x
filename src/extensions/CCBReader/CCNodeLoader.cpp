        ...
        if(!pCCBReader->isJSControlled()) {
            ...
            if(target != NULL) {
                if(selectorName.length() > 0) {
                    ...
                    blockData->mSELMenuHandler = NULL;
                    blockData->mSelectorName.initWithFormat(selectorName.c_str());
            ...
            //DuongNT: target = pCCBReader->getActionManager()->getJSController();
            BlockData * blockData = new BlockData();
            blockData->mSELMenuHandler = NULL;
            blockData->mSelectorName.initWithFormat(selectorName.c_str());
            blockData->mTarget = NULL;
        ...
        if(!pCCBReader->isJSControlled()) {
            ...
            if(target != NULL) {
                if(selectorName.length() > 0) {
				    ...
				    blockCCControlData->mSELCCControlHandler = NULL;

                    blockCCControlData->mSelectorName.initWithFormat(selectorName.c_str());
