...
void CCMenuItemLoader::onHandlePropTypeBlock(CCNode * pNode, CCNode * pParent, const char * pPropertyName, BlockData * pBlockData, CCBReader * pCCBReader) {
    if(strcmp(pPropertyName, PROPERTY_BLOCK) == 0) {
        ...
        {
			if (pBlockData->mSelectorName.length() == 0)
				((CCMenuItem *)pNode)->setTarget(pBlockData->mTarget, pBlockData->mSELMenuHandler);
            else if (pCCBReader->isJSControlled()) {
                ((CCMenuItem *)pNode)->setTarget(pCCBReader->getAnimationManager()->getJSController(), v8::String::New(pBlockData->mSelectorName.getCString())->ToObject());
            } else {
				//v8::Handle<v8::Object> func = this->getJSCCallFunc()->getObjectMemberByName(pBlockData->mTarget->getJSObjInstance(), pBlockData->mSelectorName.getCString());
				((CCMenuItem *)pNode)->setTarget(pBlockData->mTarget, v8::String::New(pBlockData->mSelectorName.getCString())->ToObject());
			}
        }
    } else {
