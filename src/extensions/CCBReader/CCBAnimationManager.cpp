...
v8::Handle<v8::Object> CCBAnimationManager::getJSController()
{
...
#ifdef _JACOS2DX    
    std::string fileName = name + ".js";
    v8::Handle<v8::Object> obj = this->getJSCCallFunc()->createObjectFromFile(fileName.c_str(), name.c_str());
    mJSController = v8::Persistent<v8::Object>::New(obj);
#endif    
}
...
CCActionInterval* CCBAnimationManager::getAction(CCBKeyframe *pKeyframe0, CCBKeyframe *pKeyframe1, const char *pPropName, CCNode *pNode)
{
    ...
    else if (strcmp(pPropName, "visible") == 0)
    {
        ...
        {
            return CCSequence::createWithTwoActions(CCDelayTime::create(duration), CCShow::create(), false);
        }
        else 
        {
            return CCSequence::createWithTwoActions(CCDelayTime::create(duration), CCHide::create(), false);
        }
    }
    else if (strcmp(pPropName, "displayFrame") == 0)
    {
        return CCSequence::createWithTwoActions(CCDelayTime::create(duration),
                                                CCBSetSpriteFrame::create((CCSpriteFrame *)pKeyframe1->getValue()), false);
    }
...
void CCBAnimationManager::runAction(CCNode *pNode, CCBSequenceProperty *pSeqProp, float fTweenDuration)
{
    ...
    if (numKeyframes > 1)
    {
        ...
        CCFiniteTimeAction *seq = CCSequence::create(actions, false);
        pNode->runAction(seq);
...
void CCBAnimationManager::runAnimationsForSequenceIdTweenDuration(int nSeqId, float fTweenDuration)
{
    ...
    CCAction *completeAction = CCSequence::createWithTwoActions(CCDelayTime::create(seq->getDuration() + fTweenDuration),
                                                                CCCallFunc::create(this, callfunc_selector(CCBAnimationManager::sequenceCompleted)), false);
...
void object_collection_func (v8::Persistent<v8::Value> object, void *parameter)
{
	object.Dispose();
}
void CCBAnimationManager::setJSDelegate(v8::Handle<v8::Object> objDelegate) 
{
	if (!mJSDelegate.IsEmpty())
		this->removeReference(mJSDelegate);
	
	mJSDelegate = v8::Persistent<v8::Object>::New(objDelegate);
	this->markReference(mJSDelegate);
	mJSDelegate.MakeWeak(NULL, &object_collection_func);
}
...
{    
	...
	if (!mJSDelegate.IsEmpty()) {
		v8::Handle<v8::Value> name = v8::String::New(mRunningSequence->getName());
		this->getJSCCallFunc()->callJSFunction(mJSDelegate, name);
	}
