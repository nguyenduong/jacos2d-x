...
/*
        v8::Handle<v8::Object> targetJSObj = getAnimationManager()->getJSController();
        
        if (!targetJSObj.IsEmpty() && !pNodeGraph->getJSObjInstance().IsEmpty()) {
            targetJSObj->Set(v8::String::New("rootNode"), pNodeGraph->getJSObjInstance());
            //pNodeGraph->getJSObjInstance()->Set(v8::String::New("controller"), targetJSObj);
            pNodeGraph->setController(targetJSObj);
        }
*/
        if (!getAnimationManager()->getJSController().IsEmpty())
            getJSCCallFunc()->callJSFunctionWithName(getAnimationManager()->getJSController(), "onDidLoadFromCCB");
    }
    return pNodeGraph;
}
CCNode* CCBReader::readNodeGraphFromData(jacos2dx::InputStream *pData, CCObject *pOwner, const CCSize &parentSize)
{	
	
	jacos2dx::ByteArray buffer(pData->available());
	pData->readByteArray(&buffer);

	unsigned char* bufferData = (unsigned char*)malloc(pData->available());
	memcpy(bufferData, buffer.getBufferData(), pData->available());
	CCData *data = new CCData(bufferData, buffer.getLength());
...
CCNode * CCBReader::readNodeGraph(CCNode * pParent) {
...
#ifdef _JACOS2DX
	//bind actions' functions to node
	if (!ccNodeLoader->getJSObjInstance().IsEmpty())
	{
		v8::Handle<v8::Object> jsActions = ccNodeLoader->getJSCCallFunc()->getObjectMemberByName(ccNodeLoader->getJSObjInstance(), "actions");
		if (!jsActions.IsEmpty()) {
			v8::Local<v8::Array> props = jsActions->GetPropertyNames();
			for (int i = 0; i < props->Length(); i++) {
				v8::Local<v8::Value> key = props->Get(i);
				v8::Local<v8::Value> item = jsActions->Get(key);
				node->getJSObjInstance()->Set(key, item);
			}
		}
	}
#endif
    ...
    if(jsControlled && node == mActionManager->getRootNode())
    {
        ...
        if (!targetJSObj.IsEmpty() && !node->getJSObjInstance().IsEmpty()) {
            targetJSObj->Set(v8::String::New("rootNode"), node->getJSObjInstance());
            node->setController(targetJSObj);
        }
    }
...
#ifdef _JACOS2DX
				v8::HandleScope handleScope;
				v8::Handle<v8::Object> targetJSObj = target->getJSObjInstance();
                v8::Local<v8::String> keyName = v8::String::New(memberVarAssignmentName.c_str());
				if (!targetJSObj.IsEmpty() && !node->getJSObjInstance().IsEmpty()) {
					targetJSObj->Set(keyName, node->getJSObjInstance());
				}
#else
