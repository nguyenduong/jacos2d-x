...
#define CCB_VIRTUAL_NEW_AUTORELEASE_CREATECCNODE_METHOD(T, CLASS_ID) virtual T * createCCNode(cocos2d::CCNode * pParent, cocos2d::extension::CCBReader * pCCBReader) { \
    T* ret = T::create(); \
	ret->assignJavaScriptObject(CLASS_ID, false); \
	return ret; \
}
...
struct BlockData {
	...
	//DuongNT added this member for JavaScript
	CCString  mSelectorName;
};
...
class CCNodeLoader : public CCObject {
    ...
    protected:
        CCB_VIRTUAL_NEW_AUTORELEASE_CREATECCNODE_METHOD(CCNode, CLS_CCNODE_ID);
