...
namespace cocos2d
{
    extern const char* getFileFromCache(const char* filename);
}
...
#ifdef _JACOS2DX
    sharedMusic().Open(cocos2d::getFileFromCache(pszFilePath), _Hash(pszFilePath));
#else
...
#ifdef _JACOS2DX        
        const char* fn = cocos2d::getFileFromCache(pszFilePath);
        if (fn) {
            pPlayer->Open(fn, nRet);
        } else
            return;
#else
