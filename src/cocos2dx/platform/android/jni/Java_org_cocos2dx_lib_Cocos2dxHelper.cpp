...
extern "C" {
   ...
   const char* getPackageNameJNI() {
	...
	void logHeap() {
        ...
        if (JniHelper::getStaticMethodInfo(t, CLASS_NAME, "logHeap", "()V")) {
            t.env->CallStaticVoidMethod(t.classID, t.methodID);
            t.env->DeleteLocalRef(t.classID);
        }
	}
	...
	void setOrientationJNI(int orientation) {
		JniMethodInfo t;

        if (JniHelper::getStaticMethodInfo(t, CLASS_NAME, "setOrientation", "(I)V")) {
